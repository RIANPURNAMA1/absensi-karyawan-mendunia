...
<div class="container-fluid px-4 py-4">
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
            <h5 class="mb-0" style="font-weight: 700; font-size: 16px;">Rekap Jadwal Shift</h5>
            <small class="text-muted">Rekap kehadiran berdasarkan jadwal shift karyawan</small>
        </div>
    </div>
        ...
        <div class="card-body p-4">
            <div class="row g-3 align-items-end">
                <div class="col-md-4">
                    ...
                    <select id="selectKaryawan" class="form-select select2">
        ...
        <div class="card border-0 shadow-sm rounded-3 mb-3">
            <div class="card-body p-3">
                <div class="d-flex flex-wrap gap-3">
                    ...
                    <div class="legend-item"><div class="legend-box bg-info"></div> Izin</div>
        ...
        <div class="card border-0 shadow-sm rounded-3">
            <div class="card-body p-4">
                ...
                <div id="emptyState" class="text-center py-5 text-muted d-none">
                    ...
                    <p class="mt-2 mb-0">Tidak ada jadwal shift di bulan ini</p>
                </div>
            </div>
        </div>
    ...
    <div id="emptyStateNoSelect" class="text-center py-5 text-muted">
        ...
        <p class="mt-2 mb-0">Pilih karyawan dan klik Tampilkan</p>
    </div>
</div>
        ...
        <div class="modal-content border-0 shadow-lg">
            ...
            <div class="modal-body p-4">
                <div class="mb-3">
                    ...
                    <br>
                    <small class="text-muted">Shift: <span id="modalShift" class="fw-bold text-dark"></span></small>
                </div>
...
<script>
const shiftColors = @json($shifts->pluck('nama_shift', 'id'));

$('#selectKaryawan, #selectBulan, #selectTahun').on('change', function() {
    if ($('#selectKaryawan').val()) {
        loadData();
    }
});

function loadData() {
    const userId = $('#selectKaryawan').val();
    const bulan = $('#selectBulan').val();
    const tahun = $('#selectTahun').val();

    if (!userId) {
        Swal.fire({ icon: 'warning', title: 'Pilih Karyawan', text: 'Silakan pilih karyawan terlebih dahulu' });
        return;
    }
    ...
    $.ajax({
        url: `/rekap-jadwal-shift/${userId}?bulan=${bulan}&tahun=${tahun}`,
        type: 'GET',
...
function renderGrid(data, bulan, tahun) {
    ...
    const karyawanName = $('#selectKaryawan option:selected').text();
    $('#rekapTitle').text(`Rekap ${karyawanName} - ${bulanNama[bulan - 1]} ${tahun}`);
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        let cellHtml = '';
        if (cellData && cellData.shifts && cellData.shifts.length > 0) {
            const shifts = cellData.shifts;
            let boxes = '';
            shifts.forEach(s => {
                boxes += `<div class="grid-cell ${s.color} ${s.text_color} status-cell" title="${s.shift_nama}: ${s.status}" data-tanggal="${dateStr}" data-shift-id="${s.shift_id || ''}" data-absensi-id="${s.absensi_id || ''}" data-status="${s.status}" data-shift-nama="${s.shift_nama}" data-initial="${s.initial}" style="cursor:pointer;">${s.initial}</div>`;
            });
...
function renderSummary(data) {
    let hadir = 0, terlambat = 0, alpa = 0, izin = 0, libur = 0, belumAbsen = 0;
    let totalShifts = 0;
...
$(document).on('click', '.status-cell', function() {
    ...
    modalData = {
        ...
        shiftId: el.data('shift-id'),
        absensiId: el.data('absensi-id'),
        status: el.data('status'),
        shiftNama: el.data('shift-nama'),
    ...
    $('#modalShift').text(modalData.shiftNama + ' (' + modalData.initial + ')');
...
function simpanStatus() {
    ...
    $.ajax({
        url: '/rekap-jadwal-shift/update-status',
        type: 'POST',
        data: {
            _token: '{{ csrf_token() }}',
            user_id: userId,
            tanggal: modalData.tanggal,
            shift_id: modalData.shiftId,
            status: status,
        },
        success: function() {
...
$(document).ready(function() {
    if ($.fn.select2) {
        $('#selectKaryawan').select2({ theme: 'bootstrap-5', width: '100%' });
    }
});
</script>
@endsection
