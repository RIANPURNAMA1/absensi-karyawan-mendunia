                    ...
                    <table class="table align-middle mb-0">
                        ...
                        <tbody>
                            @forelse($absensi as $a)
                                <tr>
                                    <td>
                                        <img src="{{ $a->user->foto_profil && file_exists(public_path('storage/foto-karyawan/' . $a->user->foto_profil)) ? asset('storage/foto-karyawan/' . $a->user->foto_profil) : asset('assets/images/avatar/avatar-1.jpg') }}" class="rounded-circle" width="40" height="40">
                                    </td>
                                    <td>{{ $a->user->nip }}</td>
                                    <td>{{ $a->user->name }}</td>
                                    <td>{{ $a->jam_masuk ? \Carbon\Carbon::parse($a->jam_masuk)->format('Y-m-d H:i') : '-' }}</td>
                                    <td>{{ $a->jam_keluar ? \Carbon\Carbon::parse($a->jam_keluar)->format('Y-m-d H:i') : '-' }}</td>
                                    <td>
                                        <span class="badge {{ $a->status == 'HADIR' ? 'bg-success' : ($a->status == 'TERLAMBAT' ? 'bg-warning' : 'bg-danger') }}">
                                            {{ $a->status ?? '-' }}
                                        </span>
                                    </td>
